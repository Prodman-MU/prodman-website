...
**Gap — this is still the biggest content hole in the whole site:** the club's actual leadership, **President Sai Harsha Sadhu and Vice President Akhil Menon** (confirmed from the MU ProdMan Core Members Application form), have **no bios and no photos** in `resources/` at all. A "Members" section that's missing its own President/VP will read as unfinished the moment anyone who knows the club looks at it. Same likely applies to any other core members beyond these four. Also, **Supriya has no last name and no contact/socials listed** — same treatment as the other three is needed before her card matches. **Action needed from the club exec team: full core-member roster with bio + superpower + contact + photo for everyone who should appear**, not just the four currently drafted.

**Photo asset issues to fix before build:** the two supplied photos are wildly inconsistent — `Muskan-Photo.jpg` is an 11.7MB, 6048×8064 iPhone portrait shot; `Supriya-Photo.jpg` is a 938KB, 3024×4032 casual photo with different lighting/background. Neither is cropped or compressed for web. **Recommend a one-line photo brief** (consistent crop ratio, plain/consistent background, target file size) sent to every member before their card gets built, plus a build-time image-optimization step regardless.
